...
class CrossCheckPipeline:
    ...
    async def process(self, item_id: str, text: str) -> ItemResult:
        ...
        if vote.total >= pc.min_votes and vote.agree * 2 > vote.total and vote.share >= pc.accept_threshold:
            return ItemResult(item_id, text, vote.label, STATUS_MAJORITY, round(vote.share, 3), r1, r2)

        arb = None
        if self.arbiter is not None and vote.total > 0:
        ...
        note = "所有模型调用失败" if vote.total == 0 else "模型分歧未解决"
        return ItemResult(item_id, text, suggestion, STATUS_HUMAN, round(vote.share, 3), r1, r2, arb, note)

    async def run(self, items: list[dict], progress: bool = True) -> list[ItemResult]:
