...
- **扫描版 PDF**：整页几乎没有文字、又铺着大图时，用本机 RapidOCR 识别（`pip install rapidocr-onnxruntime`），不调用大模型，一页大约几秒。文字版 PDF 不走识别。合成扫描页实测能读出“诉讼仲裁”“经营情况正常”
- 按字段抽取年报信息、按股票代码下载年报，这两项还没有做。一段多个标签可以在分类任务页开启“多标签”
...
**目前还不支持**：三级及以上的层级、把超长对话先摘要再分类、按字段抽取年报里的具体事项、按股票代码自动下载年报。对话、文字版年报和扫描版 PDF 的逐段分类已经可以在运行页切换；扫描件需要本机安装 RapidOCR。
...
- [x] **文档解析**：支持 PDF / Word / HTML / TXT，提取正文；PDF 保留页码；去掉目录、重复页眉和“本公司董事会保证……”这类套话；Word 表格转成文本。扫描件只识别“几乎没有文字层、且有一张图盖住页面四成以上”的页，用本机 RapidOCR，文字版 PDF 保持原样
